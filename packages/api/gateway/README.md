...
Browser shells can supply `ClientTransportHooks.openMuxSocket` through Connection. The native mux calls this physical socket factory on each attempt; it may await a fresh admission proof while retaining native framing and retry ownership. Cancellation aborts preparation, closes late returns, and ends the carrier lifetime after socket loss. Replacing a logical `openStream` carrier is unnecessary.

Host deployments can provide `ctx.gatewayAccess` and set `requireAccessPolicy: true`. Missing or rejected admission then denies network Remote access. The provider admits the original HTTP request or WebSocket upgrade, holds one finite lease per carrier, and checks calls, stream reads, `$events`, and `$events/result` before dispatch. It may project or suppress results without replacing native RPC codecs or mux framing. Lease cancellation closes the socket; hidden waterfall deliveries are removed from pending delivery accounting. The default `false` preserves the standalone Host composition. This extension does not authorize other Connection namespaces or create an identity provider.
...
- Without a deployment access provider, forwarded events reach `$on` without business-payload projection or redaction. Ordinary notifications are not replayed after reconnect; Agent-scoped waterfalls project only the top-level Agent identity needed to select the Client Context and carry their own pending lifetime.
