...
部署可以限制下载中允许包含的 Session。导出开始前必须能够读取根 Session；生成压缩包时会重新检查每个后代 Session 及其引用的附件。如果下载期间访问权限失效，流会失败，部分 ZIP 不算完成的导出。请刷新访问权限，并从仍可读取的 Session 重试。如果部署的可见性提供者不能捕获具有有限请求生命周期的读取器，导出就无法开始。
