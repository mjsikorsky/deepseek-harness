...
describe('independent native presentation leases', () => {
  it('opens two histories without moving selection and releases only its own removed view', async () => {
    ...
    expect(right.binding.session.getSnapshot().header?.createdAt).toBe(0)
    await expect(b.svc.acquireView(sid('right'), 1)).rejects.toThrow('generation differs')
    const verifiedRight = await b.svc.acquireView(sid('right'), 0)
    verifiedRight.release()
    expect(b.svc.list.getSnapshot().current).toBeUndefined()
