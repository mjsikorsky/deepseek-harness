...
|---|---|---|
| `requireAccessPolicy` | `false` | Require a deployment resource grant in addition to native browser authentication. |
...
A deployment can set `requireAccessPolicy: true` and provide `ctx.openInAppAccess` to authorize each installed-app catalog read, icon read, and directory launch after native browser authentication. A missing or denied grant returns 403 before resource access. The provider verifies its own workspace or machine capability and returns a finite lease; launch leases name one canonical absolute directory. The native handler rechecks the lease and that directory before primary, fallback, and refreshed launch attempts. Request disconnection, plugin disposal, or lease revocation ends pending authority and releases the lease. This does not terminate an application already launched or isolate the host filesystem from other local processes. The default remains unrestricted native personal mode when no provider is configured. Policy-protected icon responses use `no-store`.
...
[`src/index.ts`](src/index.ts) registers the three routes on `ctx.webServer`: `GET /open-in-app/apps` (the resolution map's keys), `GET /open-in-app/icon/<id>` (the extracted icon, cached in memory per process), and `POST /open-in-app/open` (launches the map's verified launcher directly — never a re-detection). Every route asks the composition's `connection` service for a rejection first; the native trust fence and deployment resource policy has one home in the [`src/index.ts`](src/index.ts) module comment. On top of that fence the open route validates its body at the wire: an `application/json` media type, a 64 KiB ceiling, a resolved-available catalog id, and an absolute path naming an existing directory. Resolution and icon commands run through [`@deepseek-ai/dsh-native-command`](../../util/native-command/README.md) (argv, never a shell) under their respective deadlines; PATH names go through `ctx.subprocess.resolveExecutable()` in-process.
